...
def test_create_post_no_article_link(client):
    data = {"description": "Test post description"}
    response = client.post("/api/posts/", json=data)
...
def test_create_post_not_logged_in(client):
    ...
    data = {"article_link": "http://example.com/article"}
    response = client.post("/api/posts/", json=data)
...
def test_get_post_success(client):
    ...
    post = Post(user_id=1, article_id=article.article_id, description="Test post")
    db.session.add(post)
    db.session.commit()
